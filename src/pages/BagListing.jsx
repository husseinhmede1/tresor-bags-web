...
/* ── SliderGroup outside component to avoid remount ── */
const SliderGroup = ({ label, minVal, maxVal, setMin, setMax, range, unit }) => (
    <div style={S.filterGroup}>
        ...
        <div style={S.sliderGroup}>
            <input type="range"
                min={range.min} max={maxVal || range.max}
                value={minVal || range.min}
                onChange={(e) => {
                    const v = e.target.value; setMin(v);
                    if (maxVal && Number(v) > Number(maxVal)) setMax(v);
                }} />
            <input type="range"
                min={minVal || range.min} max={range.max}
                value={maxVal || range.max}
                onChange={(e) => {
                    const v = e.target.value; setMax(v);
                    if (minVal && Number(v) < Number(minVal)) setMin(v);
                }} />
        </div>
        <div style={S.sliderValues}>
            <span>{minVal || range.min} {unit}</span>
            <span>{maxVal || range.max} {unit}</span>
        </div>
    </div>
);
...
const RevealCard = ({ children, index }) => {
    ...
    return (
        <div ref={ref} className="card-reveal-wrapper">
            {children}
        </div>
    );
};
...
const LogoWithZipper = ({ src }) => {
    ...
    return (
        <div style={{ position: "relative", display: "inline-flex", alignItems: "center", flexShrink: 0 }}>
            <img ref={useRef(null)} src={src} alt="Trésor Outlet Store"
...
const BagListing = () => {
    ...
    useEffect(() => {
        ...
        style.textContent = `
            *, *::before, *::after { box-sizing: border-box; }
            ...
            .card-reveal-wrapper.card-revealed {
                opacity: 1;
            ...
            input[type="range"] {
                ...
                background: rgba(223,169,75,0.2); outline: none; cursor: pointer;
            }
            input[type="range"]::-webkit-slider-thumb {
                -webkit-appearance: none;
                width: 20px; height: 20px; border-radius: 50%;
                background: #dfa94b; border: 3px solid rgba(223,169,75,0.35);
                box-shadow: 0 0 10px rgba(223,169,75,0.4); cursor: pointer;
            }
    ...
    const fetchBags = async () => {
        ...
        try {
            ...
            setBags(result.data); setTotalPages(result.totalPages); setTotal(result.total);
        } catch (err) { setError(err.message || "Failed to fetch bags"); }
        finally { setLoading(false); }
    };

    useEffect(() => { setPage(1); },
        [searchQuery, minPrice, maxPrice, minHeight, maxHeight, minWidth, maxWidth, minWeight, maxWeight, color, capacity]);
    useEffect(() => { fetchBags(); },
        [page, searchQuery, minPrice, maxPrice, minHeight, maxHeight, minWidth, maxWidth, minWeight, maxWeight, color, capacity]);

    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure you want to delete this bag?")) return;
        try { await deleteBag(id); fetchBags(); }
        catch (err) { alert("Failed to delete bag: " + err.message); }
    };
    ...
    const activeFilterCount = [searchQuery, minPrice, maxPrice, minHeight, maxHeight,
        minWidth, maxWidth, minWeight, maxWeight, color, capacity].filter(Boolean).length;
        ...
        <div style={S.page}>
            ...
            <div style={S.heroWrap} className="t-hero-wrap">
                ...
                <section style={S.heroSection}>
                    ...
                    <p style={S.heroSub} className="t-hero-sub">
            ...
            {totalPages > 1 && (
                <div style={S.pagination} className="t-pagination">
                    <button className="t-page-btn" onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1}
                        style={{ ...S.pageBtn, opacity: page === 1 ? 0.4 : 1 }}>← Prev</button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                        .slice(Math.max(0, page - 3), Math.min(totalPages, page + 3))
                        .map(p => (
                            <button key={p} className="t-page-btn" onClick={() => setPage(p)}
                                style={{ ...S.pageBtn, ...(p === page ? S.pageActive : {}) }}>{p}</button>
                        ))}
                    <button className="t-page-btn" onClick={() => setPage(Math.min(totalPages, page + 1))} disabled={page === totalPages}
                        style={{ ...S.pageBtn, opacity: page === totalPages ? 0.4 : 1 }}>Next →</button>
                </div>
            )}

            {loading ? (
                <div style={S.loadingBox}><div style={S.spinner} /><p style={{ color: "#A7A19A", marginTop: 16 }}>Loading bags…</p></div>
            ) : bags.length === 0 ? (
                <div style={S.emptyBox}><p style={{ fontSize: 40, marginBottom: 12 }}>🎒</p><p style={{ color: "#A7A19A" }}>No bags found. Try adjusting your filters.</p></div>
            ) : (
                <main style={S.grid} className="t-grid">
                    {bags.map((bag, index) => (
                        /* ── Each card wrapped in RevealCard for cinematic entrance ── */
                            ...
                            <div style={S.card} className="tresor-card">
                                ...
                                <div style={S.cardBody}>
                                    ...
                                    {expandedBag === bag._id && (
                                        <div style={S.details}>
                                            {[
                                                ["Height", `${bag.dimensions?.height || "N/A"} cm`],
                                                ["Width", `${bag.dimensions?.width || "N/A"} cm`],
                                                ["Depth", `${bag.dimensions?.depth || "N/A"} cm`],
                                                ["Weight", `${bag.weight || "N/A"} kg`],
                                                ["Color", bag.color],
                                                ["Capacity", bag.capacity || "N/A"],
                                            ].map(([k, v]) => (
...
const S = {
    ...
    header: {
        ...
        padding: "14px 28px", background: "rgba(7,7,7,0.94)",
        borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 0, zIndex: 100,
        backdropFilter: "blur(20px)", gap: 12,
    },
    headerRight: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: "flex-end" },
    adminBadge: {
        background: "rgba(223,169,75,0.12)", color: GOLD_L, padding: "6px 14px",
        borderRadius: 999, fontSize: 11, fontWeight: 700, letterSpacing: "0.1em",
        textTransform: "uppercase", border: `1px solid rgba(223,169,75,0.22)`, whiteSpace: "nowrap",
    },
    addBtn: {
        background: `linear-gradient(135deg,#C9A86A,${GOLD_L})`, color: TEXT,
        border: "none", padding: "9px 18px", borderRadius: 999,
        fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap",
    },
    logoutBtn: {
        background: "transparent", color: MUTED, border: `1px solid ${BORDER}`,
        padding: "9px 30px", borderRadius: 999, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap",
    },
    heroWrap: { position: "relative", minHeight: 460, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" },
    heroOverlay: {
        position: "absolute", inset: 0,
        background: "linear-gradient(180deg,rgba(7,7,7,0.55) 0%,rgba(7,7,7,0.22) 40%,rgba(7,7,7,0.22) 60%,rgba(7,7,7,0.7) 100%)",
    ...
    searchSpinner: {
        position: "absolute", right: 14, width: 16, height: 16,
        border: "2px solid rgba(223,169,75,0.2)", borderTop: `2px solid ${GOLD}`,
    ...
    grid: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 24, padding: "0 16px", marginBottom: 40, maxWidth: 1240, marginLeft: "auto", marginRight: "auto" },
    ...
    pageActive: { background: GOLD_L, color: BG, borderColor: GOLD_L },
};
